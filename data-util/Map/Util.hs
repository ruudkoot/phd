module Data.Map.Util ((!!), restrict, mapKeysAndValuesWith, restrictBySet, unionMap) where
...
-- FIXME: should be renames: restrict should 'keep'
--        only the given elements, not delete them
...
-- FIXME: should be renames: restrict should 'keep'
--        only the given elements, not delete them
restrictBySet :: Ord k => M.Map k a -> S.Set k -> M.Map k a
restrictBySet = S.foldr M.delete

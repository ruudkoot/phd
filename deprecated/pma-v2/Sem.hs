...
boundVariablesDistinct :: Expr -> Bool      -- FIXME: actually call this from somewhere
...
transform Nil
    = return Nil
transform (Cons e1 e2) -- FIXME: not the most optimal transformation if argument is a Var
    = do (y1, y2) <- fresh
         e1' <- transform e1
         e2' <- transform e2
